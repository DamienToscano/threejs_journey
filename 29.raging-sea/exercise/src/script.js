...
gui.add(waterMaterial.uniforms.uBigWavesSpeed, 'value').min(0).max(5).step(0.001).name('Big Waves Speed')
gui.addColor(debugObject, 'depthColor').name('Depth Color').onChange(() => {
    waterMaterial.uniforms.uDepthColor.value.set(debugObject.depthColor)
    renderer.setClearColor(new THREE.Color(debugObject.depthColor), 1)
})
...
gui.add(waterMaterial.uniforms.uColorMultipler, 'value').min(0).max(10).step(0.001).name('Color Multipler')
...
gui.add(waterMaterial.uniforms.uSmallWavesIterations, 'value').min(0).max(6).step(1).name('Small Waves Iterations')
...
const tick = () =>
{
    ...
    // Update time value for uniform of shader materials
